...
mutationController.mutationSchema = (req, res, next) => {
  ...
  for (let table in databaseInfo) {
    ...
    else if (columns.hasOwnProperty('id')) delete databaseInfo[table].columns.id;

    mutationSchema += `input Add${typeName}Input {\n`
    for (let columnName in columns) {
      if (columnName !== 'id' && columnName !== '_id') mutationSchema += `  ${columnName}: ${columns[columnName]}\n`
    }
    mutationSchema += '}\n\n'
...
mutationController.mutationResolver = (req, res, next) => {
  ...
  for (let table in databaseInfo) {
    ...
    for (let i = 0; i < properties.length; i++) {
        if (properties[i] !== 'id' && properties[i] !== '_id') {
          if (i === properties.length - 1) {
          mutationResolver += `$${i + 1}) RETURNING *'; \n      const values = [ `;
          } else {
  ...
  // for input: Update
    ...
    for (let i = 0; i < properties.length; i++) {
      if (properties[i] !== 'id' && properties[i] !== '_id') {
        if (i === properties.length - 1) {
        mutationResolver += `${properties[i]} = $${i + 1} WHERE _id = $${properties.length + 1} RETURNING *';\n      const values = [ `
        } else {
